...
export function buildMockPageContext(config: IPageContextConfig): any {
    const siteServerRelativeUrl = config.site.serverRelativeUrl ?? new URL(config.site.absoluteUrl).pathname;
    const webServerRelativeUrl = config.web.serverRelativeUrl ?? new URL(config.web.absoluteUrl).pathname;
    const language = config.web.language ?? getLanguageCodeFromCulture(config.cultureInfo.currentCultureName);
    ...
    return {
        ...
        cultureInfo: {
            ...config.cultureInfo,
            currentUICultureName: config.cultureInfo.currentUICultureName ?? config.cultureInfo.currentCultureName,
            isRightToLeft: config.cultureInfo.isRightToLeft ?? isRtlCulture(config.cultureInfo.currentCultureName)
        },
