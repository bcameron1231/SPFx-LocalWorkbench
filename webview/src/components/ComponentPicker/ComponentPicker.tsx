...
export const ComponentPicker: FC<IComponentPickerProps> = (props) => {
  ...
  const filteredComponents = components.filter((component) => {
    ...
    return (
      ...
      (component.description && component.description.toLowerCase().includes(filter.toLowerCase()))
    );
  });
        ...
        <div className={styles.results} id={`picker-results-${location ?? 0}`}>
          {filteredComponents.length > 0 ? (
            ...
            <Stack verticalAlign="center" horizontalAlign="center">
              <Text className={styles.noResults}>{noResultsLabel || 'No results found'}</Text>
            </Stack>
          )}
        </div>
      </Stack>
