...
export function defaultBodyForStatus(status: number): unknown {
  switch (status) {
    ...
    case 400:
      return { error: { code: 'BadRequest', message: 'Bad request' } };
    case 401:
      return {
        error: {
          code: 'Unauthorized',
    ...
    case 404:
      return { error: { code: 'NotFound', message: 'The requested resource was not found.' } };
    case 429:
      return {
        error: {
