...
import type { ApiClientType, IMockRule } from '../types';
import {
  CLIENT_TYPE_OPTIONS,
  METHOD_OPTIONS,
  STATUS_CODE_OPTIONS,
...
export async function generateStatusStubs(): Promise<IMockRule[] | undefined> {
  ...
  const statusPicks = await vscode.window.showQuickPick(
    ...
    {
      title: 'Mock Rule Generator (4/4)',
      placeHolder: 'Select status codes to generate',
      canPickMany: true,
      ignoreFocusOut: true,
    },
  );
  if (!statusPicks || statusPicks.length === 0) {
    return undefined;
  }
