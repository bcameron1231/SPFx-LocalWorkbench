...
export class StorybookServerManager {
    ...
    private async initializeStorybook(): Promise<void> {
        this.outputChannel.appendLine('Storybook not detected. Initializing Storybook configuration...');
...
`;
        ...
        await vscode.workspace.fs.writeFile(
            vscode.Uri.file(mainTs),
            Buffer.from(mainConfig, 'utf-8')
        );
    ...
    private async linkAddon(): Promise<void> {
        ...
        try {
            ...
            const extensionAddonPath = path.join(this.extensionUri.fsPath, 'packages', 'storybook-addon-spfx');
            ...
            packageJson.devDependencies['@spfx-local-workbench/storybook-addon-spfx'] = 'file:' + addonPath.replace(/\\/g, '/');
